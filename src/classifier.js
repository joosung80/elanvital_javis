...
4. DOCUMENT - 문서 분석 기능
   ...
   - google_docs_keyword_search (구글 독스 키워드 검색): "독스에서 ~ 찾아줘", "구글 독스에서 ~ 찾아줘", "구글 독스 ~ 검색해줘", "~ 문서 검색해줘", "구글 독스에서 ~ 문서 찾아줘" 등 Google Docs 전체에서 키워드로 문서 검색 요청
   ...
   - Google Docs 키워드 검색 요청 시 검색 키워드를 추출합니다. 예: "독스에서 보고서 찾아줘" → "보고서", "구글 독스에서 패스워드 문서 검색해줘" → "패스워드"
...
DOCUMENT 카테고리인 경우:
{
    ...
    "extractedInfo": {
        ...
        "documentAlias": "Google Docs 검색 요청 시 문서 별칭을 추출합니다. 예: '패스워드 문서에서 gmail 찾아줘' → '패스워드'. Google Docs 키워드 검색이나 기타 경우 빈 문자열입니다."
    }
}

중요: 일정 추가(add)의 경우 시간 정보와 내용을 분리하지 말고 원본 텍스트 전체를 보존하세요.

Google Docs 키워드 검색 시 키워드 추출 규칙:
- "구글 독스에서 [키워드] 문서 검색해줘" → 키워드: [키워드]
- "구글 독스에서 [키워드] 문서 찾아줘" → 키워드: [키워드] (예: "구글 독스에서 해커스 문서 찾아줘" → "해커스")
- "구글 독스에서 [키워드] 찾아줘" → 키워드: [키워드]
- "독스에서 [키워드] 찾아줘" → 키워드: [키워드]  
- "구글 독스 [키워드] 검색해줘" → 키워드: [키워드]
- "[키워드] 문서 검색해줘" → 키워드: [키워드]
- 키워드는 "검색해줘", "찾아줘", "문서", "구글", "독스", "에서" 등의 명령어와 전치사를 제외한 핵심 단어만 추출합니다.
...
2. "구글 독스에서 패스워드 문서 검색해줘" → searchKeyword: "패스워드" (정확함)
3. 절대로 "문서", "찾아줘", "검색해줘", "구글", "독스", "에서" 등은 키워드에 포함하지 마세요.
4. 오직 검색하려는 문서의 실제 주제나 이름만 추출하세요.
